...
class stockage
{
    private $materiel = array("ordinateur", "souris", "clavier", "ecran");
    private $base = array("ord", "sou", "cla", "ecran");
    function listeelement()
    {
        ...
        <table class="table table-striped" id="lelement">
            ...
            require('function/connect.php');
            $i = 0;
            while ($i < count($this->materiel)) {
                //exécution de la requête
                $result = mysqli_query($db, 'SELECT * FROM ' . $this->materiel[$i] . ' ORDER BY ' . $this->base[$i] . '_id DESC ');
                ...
                while ($Row = mysqli_fetch_array($result)) {
                    ...
                    <td>
                        ...
                        if (isset($_POST['' . $this->base[$i] . 'element'])) {
                            if (!empty($_POST['' . $this->base[$i] . 'libelle'])) {
                                $sql = 'UPDATE ' . $this->materiel[$i] . '
                                SET ' . $this->base[$i] . '_libelle = "' . $_POST['' . $this->base[$i] . 'libelle'] . '"
                                WHERE ' . $this->base[$i] . '_id = ' . $Row[0];
                                mysqli_query($db, $sql);
                    ...
                    if (isset($_GET['supp' . $this->base[$i] . ''])) {
                        $sql = 'DELETE FROM ' . $this->materiel[$i] . ' WHERE ' . $this->base[$i] . '_id=' . $_GET['supp' . $this->base[$i] . ''];
                        mysqli_query($db, $sql);
                        $nom = $_GET['supp' . $this->base[$i] . ''];
                        print('<meta http-equiv="refresh" content="0;URL=gestion.php?stockage#lelement">');
                    }
                $i++;
            }
    ...
    <button id="nelement" class="btn btn-primary" type="button" data-target="#Monnelement" data-toggle="collapse"
            aria-expanded="false" aria-controls="MonCollapse">Ajouter un élèment
    </button>
    ...
    function listesetup()
    {
        ...
        <table class="table table-striped" id="lsetup">
            ...
            while ($Row = mysqli_fetch_array($result)) {
                ...
                $i = 0;
                while ($i < count($this->materiel)) {
                    $req = 'SELECT ' . $this->base[$i] . '_libelle, ' . $this->base[$i] . '_code, ' . $this->base[$i] . '_marque FROM ' . $this->materiel[$i] . ' WHERE ' . $this->base[$i] . '_id = "' . $Row[$i + 2] . '"';
                    $res = mysqli_query($db, $req);
    ...
    function nsetup()
    {
        ...
        <!-- les boutons d'actions -->
        <button id="nsetup" class="btn btn-primary" type="button" data-target="#Monnsetup" data-toggle="collapse"
                aria-expanded="false" aria-controls="MonCollapse">Ajouter un setup
        </button>
        ...
        <section id="Monnsetup" class="collapse">
            ...
            <form method="post" class="form-inline">
                ...
                while ($i < count($this->materiel)) {
                    ...
                    print('
                    <div class="form-group">
                        <label class="sr-only">' . $this->materiel[$i] . '</label>
                    <select class="form-control" name="post_' . $this->base[$i] . '">');
                    require('function/connect.php');
                    $result = mysqli_query($db, 'SELECT ' . $this->base[$i] . '_id, ' . $this->base[$i] . '_code as CODE FROM ' . $this->materiel[$i] . ' WHERE NOT EXISTS(SELECT null FROM setup WHERE setup.' . $this->base[$i] . '_id = ' . $this->materiel[$i] . '.' . $this->base[$i] . '_id)');
                    while ($Row = mysqli_fetch_array($result)) {
                ...
                if (isset($_POST['nsetup'])) {
                    if (empty($_POST['post_ord']) || empty($_POST['post_sou']) || empty($_POST['post_cla']) || empty($_POST['post_ecran'])) {
                        ...
                        $i = 0;
                        while ($i < count($this->materiel)) {
                            $req = 'SELECT ' . $this->base[$i] . '_id FROM ' . $this->materiel[$i] . ' WHERE ' . $this->base[$i] . '_code = "' . $_POST['post_' . $this->base[$i] . ''] . '"';
                            $res = mysqli_query($db, $req);
                            $ver = mysqli_fetch_array($res);
                            $verlist[$i] = $ver[0];
                            $i++;

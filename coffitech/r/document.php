...
class document {
    ...
    function facture (){
        ...
        <nav class="navbar navbar-default navbar-lower">
            <div class="container-fluid">
                <ul class="nav navbar-nav">
                    <li class="nav-item">
                        <a href="gestion.php?facturec">Factures client</a>
                    </li>
                    <li class="nav-item">
                        <a href="gestion.php?facturef">Factures fournisseur</a>
                    </li>
                </ul>
            </div>
        </nav>
    ...
    function facturec (){
        $this->facture();
        $facture = new facture;
        $facture->afficher("client");
        $facture->ajouter("client");
    }
    function facturef (){
        $this->facture();
        $facture = new facture;
        $facture->afficher("fournisseur");
        $facture->ajouter("fournisseur");
    }
}
class commande {
...
class facture {
    ...
    function afficher($type){
        ...
        print('<table class="table table-striped">
                ...
                <tbody>');
        //exécution de la requête
        $result = mysqli_query($db, 'SELECT * FROM facture_'.$type.' WHERE '.$base.'_val = "N" ORDER BY '.$base.'_id DESC ');
        //lecture des resultats
        while ($Row = mysqli_fetch_array($result)) {
            $supp = '<a href="gestion.php?facture' . $min . '&amp;suppfac' . $min . '=' . $Row[0] . '" ><span class="glyphicon glyphicon-remove"></span></a>';
            $valid = '<a href="gestion.php?facture' . $min . '&amp;validfac' . $min . '=' . $Row[0] . '" ><span class="glyphicon glyphicon-ok"></span></a>';
            print('
                <tr id="#'.$base.''.$Row[0].'">
                <form method="post" id="mc'.$min.'' . $Row[0] . '" class="form-inline">
                <td>
                    <a id="lifacture" href="gestion.php?facture' . $min . '&amp;lifac' . $min . '=' . $Row[0] . '">' . $Row[1] . '
                    </a>
                </td>');
            print('<td>' . $Row[2] . '
            ...
            print('<td>
                    ...
                    <section class="f' . $Row[0] . ' collapse" >
                        <button name="mf'.$min.'' . $Row[0] . '" type="submit" class="btn btn-info">Modifier</button>
                    </section>
                </td>');
            if (isset($_POST['mf'.$min.'' . $Row[0] . ''])) {
                if (!empty($_POST[''.$base.'titre']) && !empty($_POST[''.$base.'date']) && !empty($_POST[''.$base.'client'])) {
                    $res = mysqli_query($db, 'SELECT '.$base2.'_id FROM '.$type.' WHERE '.$cfname.' = "'.$_POST[''.$base.'client'].'" ');
                    $ver = mysqli_fetch_array($res);
                    $sql = 'UPDATE facture_'.$type.'
                                SET '.$base.'_titre = "' . $_POST[''.$base.'titre'] . '",'.$base.'_date = "' . $_POST[''.$base.'date'] . '", '.$base2.'_id = "' . $ver[0] . '" 
                                WHERE '.$base.'_id = ' . $Row[0];
                    mysqli_query($db, $sql);
                    print('<meta http-equiv="refresh" content="0";URL=gestion.php?facture'.$min.'">');
                } else {
                    print('
                            <div class="alert alert-danger">
                                <strong>Danger!</strong> Vous n\'avez pas remplit tous les champs
        ...
        if (isset($_GET['suppfac' . $min . ''])) {
            $sql = 'UPDATE facture_'.$type.' SET '.$base.'_val = "C"  WHERE '.$base.'_id=' . $_GET['suppfac' . $min . ''];
            mysqli_query($db, $sql);
            print('<meta http-equiv="refresh" content="0;URL=gestion.php?facture' . $min . '">');
        }
        if (isset($_GET['validfac' . $min . ''])) {
            $sql = 'UPDATE facture_'.$type.' SET '.$base.'_val = "Y"  WHERE '.$base.'_id=' . $_GET['suppfac' . $min . ''];
            mysqli_query($db, $sql);
            print('<meta http-equiv="refresh" content="0;URL=gestion.php?facture' . $min . '">');
        }
        //libération des ressources
        ...
        if (isset($_GET['lifac' . $min . ''])) {
            print('<table class="table table-striped">
                ...
                <tbody>');
            $result = mysqli_query($db, 'SELECT * FROM ligne_facture_' . $type . ' WHERE ' . $base . '_id = "'. $_GET['lifac' . $min . ''].'" ORDER BY l' . $base . '_id DESC ');
            //lecture des resultats
            $i = 1;
            while ($Row = mysqli_fetch_array($result)) {
                $supp = '<a href="gestion.php?facture' . $min . '&amp;supplifac' . $min . '=' . $Row[0] . '" ><span class="glyphicon glyphicon-remove"></span></a>';
                print('<tr><form method="post" id="mlf'.$min.'' . $Row[0] . '" class="form-inline">');
                print('<td>' . $i . '</td>');
                if($type == "client") {
                    $res = mysqli_query($db, 'SELECT setup_code FROM setup WHERE setup_id = "' . $Row[1] . '" ');
                    $ver = mysqli_fetch_array($res);
                    print('<td>' . $ver[0] . '');
                    $listres = mysqli_query($db, 'SELECT setup_code FROM setup WHERE etat = "disponible"');
                    print('<section class="lif' . $Row[0] . ' collapse" >   
                            <div class="form-group">
                                <select class="form-control" value="' . $ver[0] . '" name="l'.$base.'setup">');
                    while ($elem = mysqli_fetch_array($listres)) {
                        print('<option>' . $elem[0] . '</option>');
                    ...
                    print('<td>' . $Row[1] . '
                            <section class="lif' . $Row[0] . ' collapse" >   
                            <div class="form-group">
                                <input type="text" name="l'.$base.'libelle" class="form-control" value="' . $Row[1] . '" placeholder="Entrer le libelle">
                            </div>
                            </section>
                ...
                print('<td>' . $total . '</td>');
                print('<td>
                    <a data-target=".lif' . $Row[0] . '" data-toggle="collapse" aria-expanded="false" aria-controls="MonCollapse"><span class="glyphicon glyphicon-edit"></span></a>
                    <section class="lif' . $Row[0] . ' collapse" >
                        <button name="mlf'.$min.'' . $Row[0] . '" type="submit" class="btn btn-info">Modifier</button>
                    </section>
                </td>');
                if (isset($_POST['mlf'.$min.'' . $Row[0] . ''])) {
                    if($type == "client") {
                        if (!empty($_POST['l' . $base . 'setup']) && !empty($_POST['l' . $base . 'qte'])) {
                            $res = mysqli_query($db, 'SELECT setup_id FROM setup WHERE setup_code = "' . $_POST['l' . $base . 'setup'] . '" ');
                            $ver = mysqli_fetch_array($res);
                            $sql = 'UPDATE ligne_facture_'.$type.'
                                SET setup_id = "'.$ver[0].'",l'.$base.'_qte = "' . $_POST['l'.$base.'qte'] . '" 
                                WHERE l'.$base.'_id = ' . $Row[0];
                            mysqli_query($db, $sql);
                            print('<meta http-equiv="refresh" content="0;URL=gestion.php?facture'.$min.'&amp;li'.$base.'=' . $_GET['lifac' . $min . ''] . '">');
                        }else{
                            print('</section>
                                <div class="alert alert-danger">
                                    <strong>Danger!</strong> Vous n\'avez pas remplit tous les champs
            ...
            print('<tr><form method="post" id="nlf'.$min.'' . $_GET['lifac' . $min . ''] . '" class="form-inline">');
            print('<td></td>');
            print('<td>');
            print('<div class="form-group">');
            if($type == "client") {
                $listres = mysqli_query($db, 'SELECT setup_code FROM setup WHERE etat = "disponible" && not exists (SELECT setup_id FROM ligne_facture_'.$type.' where setup.setup_id = ligne_facture_'.$type.'.setup_id && facc_id = "' . $_GET['lifac' . $min . ''] . '") ');
                print('<select class="form-control" name="post_l'.$base.'setup">');
                while ($elem = mysqli_fetch_array($listres)) {
                    print('<option>' . $elem[0] . '</option>');
                }
            ...
            print('<td></td>');
            print('<td></td>');
            print('<td><button name="nlf'.$min.'' . $_GET['lifac' . $min . ''] . '" type="submit" class="btn btn-info">Ajouter</button></td>');
            if (isset($_POST['nlc'.$min.'' . $_GET['lifac' . $min . ''] . ''])) {
                if($type == "client") {
                    if (!empty($_POST['post_l' . $base . 'setup']) && !empty($_POST['post_l' . $base . 'qte'])) {
                        $res = mysqli_query($db, 'SELECT setup_id FROM setup WHERE setup_code = "' . $_POST['post_l' . $base . 'setup'] . '" ');
                        $ver = mysqli_fetch_array($res);
                        $sql = 'INSERT INTO ligne_facture_'.$type.' set setup_id = "' . $ver[0]. '",l'.$base.'_qte = "' . $_POST['post_l'.$base.'qte']. '",'.$base.'_id = "'. $_GET['lifac' . $min . ''].'", tva_id = "1"';
                        mysqli_query($db, $sql);
                        print('<meta http-equiv="refresh" content="0;URL=gestion.php?facture' . $min . '&amp;li'.$base.'=' . $_GET['lifac' . $min . ''] . '">');
                    }else{
                        print('</section>
                    ...
                    if (!empty($_POST['post_l' . $base . 'libelle']) && !empty($_POST['post_l' . $base . 'qte'])&& !empty($_POST['post_l' . $base . 'puht'])) {
                        $sql = 'INSERT INTO ligne_facture_'.$type.' set l'.$base.'_libelle = "' . $_POST['post_l' . $base . 'libelle'] . '",l'.$base.'_qte = "' . $_POST['post_l'.$base.'qte']. '",'.$base.'_id = "'. $_GET['lifac' . $min . ''].'", tva_id = "1", pu_ht = "'.$_POST['post_l' . $base . 'puht'].'"';
                        mysqli_query($db, $sql);
                        print('<meta http-equiv="refresh" content="0;URL=gestion.php?facture' . $min . '&amp;li'.$base.'=' . $_GET['lifac' . $min . ''] . '">');
                    }else{
                        print('</section>
        ...
        if (isset($_GET['supplifac' . $min . ''])) {
            $sql = 'DELETE FROM ligne_facture_'.$type.' WHERE l'.$base.'_id=' . $_GET['supplifac' . $min . ''];
            mysqli_query($db, $sql);
            print('<meta http-equiv="refresh" content="0;URL=gestion.php?facture' . $min . '">');
        }
        print('</br>');
    ...
    function ajouter($type){
        ...
        print('
        <button id="nfacture" class="btn btn-primary" type="button" data-target="#Mafacture" data-toggle="collapse"
                aria-expanded="false" aria-controls="MonCollapse">Ajouter une facture
        </button>');
        print('
        <section id="Mafacture" class="collapse">
            </br>
            <form method="post" class="form-inline">
                <fieldset>
                    <legend>Facture '.$type.'</legend>
                    <div class="form-group">
                        <input type="text" name="post_'.$base.'titre" class="form-control" placeholder="Entrer le libelle">
                    </div>');
        $listres = mysqli_query($db, 'SELECT '.$cfname.' FROM '.$type.'');
        ...
        print('</fieldset>
                ...
                <button name="nf'.$min.'" type="submit" class="btn btn-primary btn-block">Ajouter</button>');
        if (isset($_POST['nf'.$min.''])) {
            if (!empty($_POST['post_'.$base.'titre']) && !empty($_POST['post_'.$base.'client'])){
                $req = 'SELECT '.$base.'_id FROM facture_'.$type.' WHERE '.$base.'_titre = "' . $_POST['post_'.$base.'titre'] . '" ';
                $res = mysqli_query($db, $req);
                $donnee = mysqli_fetch_array($res);
                //libération des ressources
                mysqli_free_result($res);
                if ($donnee[''.$base.'_id'] == '') {
                    $req = 'SELECT MAX('.$base.'_id) FROM facture_'.$type.'';
                    $result = mysqli_query($db, $req);
                    $code = mysqli_fetch_array($result);
                    $req = 'SELECT '.$base.'_code FROM facture_'.$type.' WHERE '.$base.'_id = ' . $code[0] . ' ';
                    $result = mysqli_query($db, $req);
                    $code = mysqli_fetch_array($result);
                    mysqli_free_result($result);
                    $code = substr($code[0], -2);
                    ...
                    //libération des ressources
                    mysqli_free_result($res);
                    $sql = 'INSERT INTO facture_'.$type.' set '.$base.'_code = "' . strtoupper($base) . $code . '",'.$base.'_titre = "' . $_POST['post_'.$base.'titre']. '",'.$base.'_date = "' .date("Y-m-d"). '", '.$base2.'_id = "'.$donnee[0].'", '.$base.'_val = "N"';
                    mysqli_query($db, $sql);
                    print('<meta http-equiv="refresh" content="0";URL=gestion.php?facture' . $min . '">');
                    print('</section><div class="alert alert-success">
                                        <strong>Success!</strong> Facture ajouté avec succès
                                    </div>');
                }else{
                    print('</section><div class="alert alert-danger">
                                    <strong>Danger!</strong> Une facture porte déjà ce libelle : '.$donnee[0].'
                                    </div>');
                }
            }else{
                print('</section>
